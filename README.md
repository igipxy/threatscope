...
- Size-limited SQLite scan history with URL query-string redaction
...
The API runs at **http://localhost:8000**; interactive documentation is at **http://localhost:8000/docs**. Without an access token, ThreatScope accepts only direct loopback requests and rejects forwarded or remote clients.

For a private remote deployment, generate a strong random token and set it in **backend/.env**:

~~~env
THREATSCOPE_ACCESS_TOKEN=replace-with-a-long-random-secret
SCAN_REQUESTS_PER_MINUTE=20
MAX_CONCURRENT_SCANS=4
MAX_STORED_SCANS=500
~~~

Remote API clients must send the token in the **X-ThreatScope-Key** header. Do not embed this token in a publicly served frontend bundle. For browser access, keep the backend private behind an authenticated reverse proxy and configure the proxy to supply the backend credential.
...
- Remote access fails closed unless a server-side access token is configured.
- URL query strings are analyzed but are not retained in scan results or history.
- Scan concurrency, request rates, and stored history are bounded by default.
